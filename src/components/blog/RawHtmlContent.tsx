...
<script>${RESIZE_JS.replace('__FRAME_ID__', frameId)}<\/script>
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      const data = e.data;
      if (e.source !== ref.current?.contentWindow || !data || data.source !== 'kaua-post-frame' || data.id !== frameId) return;
      const h = Number(data.height);
      if (Number.isFinite(h) && h > 0) setHeight(Math.min(100000, Math.ceil(h)));
    };
