...
export async function getLatestYouTubeVideo(): Promise<LatestYouTubeState> {
  try {
    ...
    // A blocked page or malformed feed is not evidence that the channel is empty.
    if (!video) return { status: /<feed[\s>]/i.test(feed) && !/<entry[\s>]/i.test(feed) ? 'empty' : 'unavailable', channelUrl: CHANNEL_URL };
